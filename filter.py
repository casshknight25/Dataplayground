...
if data =='Broker Data':
    ...
    st.header("Data Analysis")
    st.info("We have now sufficiently prepared the data so that the information requested can be provided. In this exercise you will create a pivot table to show the average notification times and claim lifecycles split by broker account nunmber")
    df = df.drop_duplicates(subset = "Claim reference")
    df = df.astype({"Claim reference": 'str'})
    df = df[df['Claim reference'].map(len) > 7]
    df = df.dropna()
